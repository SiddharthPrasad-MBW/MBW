...
variable "existing_configured_tables" {
  description = "Map of existing configured tables: table_id -> {name: table_name}. Use discover-configured-tables.py to generate."
  type = map(object({
    name = string
  }))
  default = {}
...
variable "id_namespace_arn" {
  description = "ARN of the Entity Resolution ID namespace to associate"
  type        = string
